...
text = Fore.WHITE + '\033[1m' + \
    "CORRI CONSTRUCTION COMPANY CONTRACTORS PAGE" + '\033[0m'
...
time.sleep(2)
type_print(
    " Failure to input your NAME correctly may result in your pay being delayed.\n")
type_print(" Our system uses your " + Fore.GREEN +
           "name and employee number" + Fore.WHITE + " to find you so it is important")
type_print(" that you enter the FULL NAME you provided for our records.")
type_print(" e.g. " + Fore.GREEN + "Mark Jenkins" + Fore.WHITE + ", although " + Fore.GREEN +
           "mark jenkins " + Fore.WHITE + "and " + Fore.GREEN + "MARK JENKINS " + Fore.WHITE + "are also ok.\n")
time.sleep(3)
...
while True:
    ...
    if first_name.strip() == "":     # code found at GeeksforGeeks
        type_print(
            Fore.RED + " First name is required. "
            "Please enter a valid first name.")
        continue
    elif not first_name.isalpha():   # code found at GeeksforGeeks
        type_print(
            Fore.RED + " First name should not contain numbers  " 
            "or symbols. Enter a valid first name.")
        continue
    last_name = input(" Enter your Last Name: ")
    if last_name.strip() == "":
        type_print(
            Fore.RED + " Last name is required. Please enter "
            "a valid First & Last name.")
        continue
    elif not last_name.isalpha():
        type_print(
            Fore.RED + " Your name should not contain numbers " 
...
# Profession confirmed by selecting a letter.


def get_profession_choice():
    """
    Dictionary to map profession codes to profession
    names and rate of pay to each profession
    """
    professions = {
    ...
    while True:
        ...
        Prompt user to enter the letter corresponding 
        to their choice
...
if __name__ == "__main__":
    ...
    time.sleep(0.5)
    type_print(Fore.WHITE +
           " This is for the months of August (08) and September(09) only.\n")
    time.sleep(0.5)
...
# User enters date, days and hours worked

while True:
    try:
        start_date_str = input(
            Fore.WHITE + " Enter the start date (DD-MM-YYYY): ")
        end_date_str = input(Fore.WHITE + " Enter the end date (DD-MM-YYYY): ")
        days_worked = int(
            input(Fore.WHITE + " Enter the number of days worked: "))
        ...
        if days_worked != calculated_days_worked:
            print(
                Fore.RED + " Error: The dates entered and the number of days worked do not match.")
            continue

        if worked_too_many_hours(days_worked, hrs):
            print(Fore.RED + " Error: You cannot enter more than 13 hours per day for a total of " +
                  str(days_worked) + " days.")
        else:
            print(" You have entered " + str(hrs) +
                  " hours for " + str(days_worked) + " days.")
            break
    except ValueError:
        print(
            Fore.RED + " Error: Check your dates match, your days add up and/or your hours.")
...
type_print((f" From {start_date.strftime('%d-%m')}") + " " + (f"to {end_date.strftime('%d-%m')}") +
...
while True:
    exit = input(
        Fore.GREEN + " Information Complete. Select y to submit or n to continue (y/n): ").lower()
